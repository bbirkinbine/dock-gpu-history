...
### 6. Sandbox check (gates the App Store path) — DONE 2026-07-17
Verified locally without full Xcode: ad-hoc-signed the dev build WITH
Resources/GPUDockHistory.entitlements (app-sandbox genuinely enforced — a
container was created), then ran the sampler under GPU load — it returned
94/99/99/99/99/99. The IORegistry read survives the sandbox, so the MAS path is
viable (Developer ID fallback no longer forced). Recorded in
...
- Done: the details/settings window (App Review 4.2 mitigation) — built as Option A per Brian's sign-off; the "windowless" rule in CLAUDE.md was amended to allow one optional secondary window. Window rendering/interaction still needs Brian's visual check.
...
- [x] 6. Sandbox verification result recorded — sandbox does NOT block the IORegistry read (verified locally via ad-hoc + entitlement, container enforced, 94/99% under load); MAS path viable
- [ ] 7. App Store prerequisites (partial):
  - [x] App icon — full AppIcon.appiconset generated by `scripts/make-icon.swift` (filled green GPU-history area chart, macOS squircle tile), wired into project.yml/Info.plist; structure validated locally (actool/xcodebuild compile is a CI gate)
  - [x] Privacy policy page drafted (`docs/privacy-policy.md`, Pages-ready) + store metadata/review notes drafted (`docs/STORE_LISTING.md`)
  - [ ] `DEVELOPMENT_TEAM` in project.yml — needs Brian's Team ID + go
  - [x] 4.2 mitigation — optional details/settings window built (Option A);
    CLAUDE.md windowless rule amended. Compiles + headless verify passes;
    window rendering/interaction still needs Brian's visual check
  - [ ] Everything requiring Brian's Apple account: enrollment, App Store Connect record, screenshots, upload, submit
